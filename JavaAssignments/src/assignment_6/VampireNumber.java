package assignment_6;

import java.util.Arrays;
import java.util.HashSet;

public class VampireNumber {
    private static int numLength(long num){
        return Long.toString(Math.abs(num)).length();
    }

    private static boolean fangCheck(long num,long fang1,long fang2){

        if(Long.toString(fang1).endsWith("0") && Long.toString(fang2).endsWith("0")){
            return false;
        }
        int numLength = numLength(num);
        //System.out.println(numLength(num/2));
        if(numLength(fang1) != numLength/2 || numLength(fang2) != numLength/2 ){
            return false;
        }
       // System.out.println("hi");
        byte[] numBytes = Long.toString(num).getBytes();

        byte[] fangBytes = (Long.toString(fang1) + Long.toString(fang2)).getBytes();

        Arrays.sort(numBytes);
        Arrays.sort(fangBytes);
        return Arrays.equals(numBytes, fangBytes);
    }
    public static void main(String[] args) {

        HashSet<Long> vamps = new HashSet<Long>();
        for(long i = 10; vamps.size()<=100; i++ ){

            if((numLength(i) % 2) != 0) {

                i = i * 10 - 1; continue;
            }

            for(long fang1 = 2; fang1 <= Math.sqrt(i) + 1; fang1++){

                if(i % fang1 == 0){
                    long fang2 = i / fang1;

                    if(fangCheck(i, fang1, fang2) && fang1 <= fang2){

                        vamps.add(i);
                        System.out.println(i + ": [" + fang1 + ", " + fang2 +"]");
                    }

                }
            }
        }
    }
}
